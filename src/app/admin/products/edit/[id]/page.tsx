            ...
            fetch(`/api/products/${productId}`)
                ...
                .then(data => {
                    ...
                    setForm({
                        ...
                        priceRangeMax: data.priceRangeMax?.toString() || "",
                        pricePerDay: data.pricePerDay?.toString() || "",
                        pricePerHour: data.pricePerHour?.toString() || "",
                        packagingFee: data.packagingFee?.toString() || "0",
                        handlingFee: data.handlingFee?.toString() || "0",
                        setupFee: data.setupFee?.toString() || "0",
                        totalUnits: data.totalUnits?.toString() || "1",
                        minOrderQty: data.minOrderQty?.toString() || "1",
                        condition: data.condition || "excellent",
                        installTime: data.installTime?.toString() || "0",
                        dismantleTime: data.dismantleTime?.toString() || "0",
                        cleaningTime: data.cleaningTime?.toString() || "0",
                    ...
                    if (data.media) {
                        setMediaFiles(data.media.map((m: any) => ({
                            id: m.id, type: m.type, url: m.url, originalName: m.alt || m.type, size: 0
                        })));
                    }
                    if (data.documents) {
                        setDocumentFiles(data.documents.map((d: any) => ({
                            id: d.id, type: d.type, url: d.url, originalName: d.name, size: d.size || 0
                        })));
                    }
                    if (data.safetyCertificates) {
                        setCertificates(data.safetyCertificates.map((c: any) => ({
                            certName: c.certName || "",
                    ...
                    if (data.installationGuides) {
                        const install = data.installationGuides.find((g: any) => g.guideType === "install");
                        if (install) {
                            setInstallGuide({
                                content: install.content || "",
                                requiredManpower: install.requiredManpower?.toString() || "",
                                estimatedTime: install.estimatedTime || "",
                                toolsRequired: install.toolsRequired || ""
                            });
                        }
                        const dismantle = data.installationGuides.find((g: any) => g.guideType === "dismantle");
                        if (dismantle) {
                            setDismantleGuide({
                                content: dismantle.content || "",
                                requiredManpower: dismantle.requiredManpower?.toString() || "",
                                estimatedTime: dismantle.estimatedTime || "",
                                toolsRequired: dismantle.toolsRequired || ""
                            });
                        }
                    }
                })
    ...
    const handleDocumentSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (!uploadRes.ok) throw new Error("Failed to upload document to S3");
        ...
        <div className="max-w-4xl">
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    ...
                    <p className="text-[var(--color-slate)] text-sm mt-0.5">Update equipment details</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="relative group">
                                    <select value={form.categoryId} onChange={(e) => updateField("categoryId", e.target.value)}
                                        ...
                                        <option value="" className="bg-[var(--color-navy-dark)] text-white">Select a category...</option>
                                        {categories.map((cat) => (
                                            <option key={cat.id} value={cat.id} className="bg-[var(--color-navy-dark)] text-white">{cat.name}</option>
                                        ))}
                                    </select>
                        ...
                        <div>
                            ...
                            <div className="relative group">
                                <select value={form.priceType} onChange={(e) => updateField("priceType", e.target.value)}
                                    className="w-full px-4 py-3 rounded-lg bg-[var(--color-navy-lighter)] border border-[var(--color-border-subtle)] text-[var(--color-warm-white)] focus:border-[var(--color-gold)] focus:outline-none transition-colors text-sm appearance-none cursor-pointer pr-10">
                                    <option value="daily" className="bg-[var(--color-navy-dark)] text-white">Daily Rate (Per Unit / Per Day)</option>
                                    <option value="job" className="bg-[var(--color-navy-dark)] text-white">Fixed Job Cost (One-time flat rate project)</option>
                                    <option value="custom" className="bg-[var(--color-navy-dark)] text-white">Custom Rate (TBD / Request required)</option>
                                </select>
                                <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--color-gold)] pointer-events-none group-hover:scale-110 transition-transform" />
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="relative group">
                                    <select value={form.unit} onChange={(e) => updateField("unit", e.target.value)}
                                        ...
                                        {UNIT_OPTIONS.map((opt) => (
                                            <option key={opt.value} value={opt.value} className="bg-[var(--color-navy-dark)] text-white">{opt.label}</option>
                                        ))}
                                    </select>
                                    <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--color-gold)] pointer-events-none group-hover:scale-110 transition-transform" />
                                </div>
                        ...
                        <div>
                            ...
                            <input type="number" min="1" value={form.minOrderQty} onChange={(e) => updateField("minOrderQty", e.target.value)}
                                placeholder="e.g. 5"
                                className="w-full px-4 py-3 rounded-lg bg-[var(--color-navy-lighter)] border border-[var(--color-border-subtle)] text-[var(--color-warm-white)] placeholder:text-[var(--color-slate)] focus:border-[var(--color-gold)] focus:outline-none transition-colors text-sm" />
                            <p className="text-[10px] text-[var(--color-slate)] mt-1">Minimum units a client must order. Shown on the product page.</p>
                        </div>
                        <div>
                ...
                <AvailabilityTimeline productId={realProductId || productId} />
                ...
                <div className="flex items-center justify-between pt-2 pb-8">
                    ...
                    <button type="submit" disabled={saving || success}
                        ...
                        {saving ? (
                            <><div className="w-4 h-4 border-2 border-[var(--color-navy)] border-t-transparent rounded-full animate-spin" /> Saving...</>
